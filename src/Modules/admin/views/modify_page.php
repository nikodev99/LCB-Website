...
$content = Model::getPage(['url' => $params['slug']]);

$redirection_url = $router->url('admin.page.modificator', ['slug' => $params['slug']]);

$condition = ['url' => trim($params['slug'])];

$page = [];
if (!$post->isEmpty()) {
    $page = $post->get('page_content') ?? [];
    if (!empty($page) && is_array($page)) {
        $url = Model::getPage($condition);
        ...
        if (in_array('contact', $page)) {
            $contact = Model::getContact($condition);
            if (is_null($contact)):
                Model::insertContact([
                    'description'   =>  'In vitae nisi aliquam, scelerisque leo a, volutpat sem. Viva mus rutrum dui fermentum eros hendrerit, id lobortis leo volutpat.',
                    'description_ang'   =>  'In vitae nisi aliquam, scelerisque leo a, volutpat sem. Viva mus rutrum dui fermentum eros hendrerit, id lobortis leo volutpat.',
                    'address'   =>  'Avenue  Amilcar cabral-Brazzaville Congo',
                    'tel'   => '(+242) 06 700 48 48 / 05 717 11 11',
                    'email' =>  'contact@lcb-bank.com',
                    'url'       =>  $url->url,
                    'url_ang'   =>  $url->url_ang,
                    'url_id'    =>  $url->id
                ]);
        ...
        AppFunction::successMessage("La page a bien été modifier. Pensez à modifier le contenu de la page en $lien et de mettre la page en ligne au terme de la modification", $redirection_url);
    }else {
        AppFunction::errorMessage('Page non enregistré. Erreur rencontrer lors de la création de la page', $redirection_url);
    }
}
